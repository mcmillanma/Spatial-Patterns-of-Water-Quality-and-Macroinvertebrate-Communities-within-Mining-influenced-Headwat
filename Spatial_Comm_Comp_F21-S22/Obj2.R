...
#CRO
cro <- filter(c.df, Stream == "CRO") %>%
  select(-c(1, 2, 4, 5, 6))
cro <- cor(cro, method = c("spearman"), use = "complete.obs") #cro <- rcorr(as.matrix(cro), type = "spearman")
...
#EAS
eas <- filter(c.df, Stream == "EAS") %>%
  select(-c(1, 2, 4, 5, 6))
eas <- cor(eas, method = c("spearman"), use = "complete.obs") #eas <- rcorr(as.matrix(eas), type = "spearman")
...
#FRY
fry <- filter(c.df, Stream == "FRY") %>%
  select(-c(1, 2, 4, 5, 6, 64, 65))
fry <- cor(fry, method = c("spearman"), use = "complete.obs") # fry <- rcorr(as.matrix(fry), type = "spearman") 
...
#LLW
llw <- filter(c.df, Stream == "LLW") %>%
  select(-c(1, 2, 4, 5, 6))
llw <- cor(llw, method = c("spearman"), use = "complete.obs") #llw <- rcorr(as.matrix(llw), type = "spearman")
...
#ROL
rol <- filter(c.df, Stream == "ROL") %>%
  select(-c(1, 2, 4, 5, 6, 44))
...
write.csv(cor.scxstream, file="cor.dist.metric.stream.s22.csv", sep = ",")
...
reg <- left_join(metrics, chem, by = c("Site" = "site")) 
reg <- left_join(reg2, hab, by = "Site")
...
box.rich.SCxstream <- ggplot(reg, aes(x=Stream.y, y=rich.SC, group = interaction(Season, Stream), color = Season))+
  stat_boxplot() +
  theme_classic() +
  ...
  stat_compare_means(method = "kruskal", label.x.npc = "left", label.y.npc = "top") +
  stat_compare_means(label = "p.signif", method = "kruskal.test", label.x.npc = "left", label.y.npc = "top") 
box.rich.SCxstream

png("box.rich.SCxstream.png", width = 900, height = 450)
plot(box.rich.SCxstream)
dev.off()
...
scxdistancexstream <- ggplot(chem ,aes(x=dist.d, y=sc.uScm))+
  geom_point(aes(color=Stream.Type))+
  # geom_jitter() +
  stat_smooth(method = "lm", linetype = 2) +
  stat_cor(method = "spearman") +
  stat_regline_equation(label.y = 1400) + #label.y = 
  #ylim(0,8) +
  #xlim(0, 2500) +
