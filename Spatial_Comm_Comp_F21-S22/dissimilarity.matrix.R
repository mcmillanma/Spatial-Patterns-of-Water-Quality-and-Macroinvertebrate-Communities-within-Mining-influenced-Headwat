...
write.csv(Similar, file="Similarity.S22.csv", sep = ",")
...
Similar.plot <- Similar %>% ggplot(aes(x=dis.simm ,y=SIM, group = Season, color = Season))+
  geom_point()  +
  stat_smooth(method = "lm", linetype = 2) +
  ...
  ylab("Simmilarity") +
  ggtitle("Community Simmilarity vs Distance Between Samples") +
  theme(plot.title = element_text(hjust = 0.5, size = 15, face = "bold")) +
  facet_wrap("Stream")
Similar.plot
